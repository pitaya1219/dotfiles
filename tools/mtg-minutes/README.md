...
| `mtg-setup` | 会議の前提(音声デバイス / 複数出力装置 / OBS)を点検して整える。`mtg` が自動で呼ぶ |
...
- `mtg-setup` 相当の点検を回し、必要なら OBS を起動する(§前提の点検)
...
- 点検で OBS を起動していたら、最後に OBS を終了する
...
`mtg` が起動時に自動で回すので、普段このコマンドを直接叩く必要はない。会議前に
単体で確かめたいときと、`mtg` を使わずに OBS だけ落としたいとき用。

```bash
mtg-setup              # 点検して直す(OBS を起動する)
mtg-setup --check      # 点検だけ。sudo も OBS の起動もしない
mtg-setup --teardown   # OBS を終了する(会議後)
mtg-setup --no-obs     # OBS には触れず、音声デバイスと出力経路だけ見る
```

`mtg` 側のオプション:

```bash
mtg --no-preflight     # 点検をしない
mtg --keep-obs         # 終了時に OBS を終了しない
```

## 何を見ているか

見ているのは、過去に「録音は完走したのに中身が無い」を起こした3点だけ。会議が
終わるまで気づけない失敗なので、ここで止める価値がある。

| 見るもの | 壊れ方 | 直し方 |
|----------|--------|--------|
| BlackHole 2ch / 16ch がデバイス一覧に居るか | helper プロセスがメモリ逼迫で刈られると、デバイスごと消える | `sudo killall coreaudiod`(点検が sudo を聞いて実行する) |
| 複数出力装置「会議用」に BlackHole 16ch が入っているか | デバイス不在の間に構成から外され、その状態が保存される | Audio MIDI 設定でチェックを入れ直す(**GUI でしか直せない**) |
| OBS が起動し BlackHole 2ch へモニタリングしているか | 起動していない / マイク再接続後にモニタリングが復帰していない | OBS を起動する。復帰漏れは OBS でモニタリングを OFF→ON |

前の2つは片側のトラックが丸ごと無音になるので、直らなければ `mtg` は起動しない
(承知のうえで進めるなら `--no-preflight`)。OBS まわりは警告だけで先に進む。
自分側が落ちても相手側の録音と議事録は成立するのと、判定材料が OBS のログという
間接的なものだからでもある。

## OBS を会議のあいだだけ起動する

**これがこのコマンドの主目的。** OBS を常駐させておくとメモリを食い、その圧力で
アイドルな 16ch の helper が jetsam に刈られる = 表の1行目に戻る。会議のたびに
起動して終わったら落とす、という形にすると原因の側が消える。

終了させるのは**点検の時点で動いていなかった場合だけ**。配信や録画で自分で開いて
いた OBS を勝手に落とさないため。`mtg-setup --teardown` は明示的に叩かれた以上
意図と見なして無条件に終了する。

`mtg` からの終了は**指示するだけで待たない**。会議の後は議事録の生成が続くので、
OBS が閉じるまでの数秒をそこに足す理由がない(`mtg` は `mtg-minutes` に execvp
するので、待っても結果を見る先が無い)。引き換えに、終了確認ダイアログなどで
閉じられなかった場合は誰も気づけない。終了を見届けたいときは
`mtg-setup --teardown` を使う。

## 判定の根拠に OBS のログを使っている

OBS WebSocket は無効(`server_enabled: false`)なので、状態は
`~/Library/Application Support/obs-studio/logs/` の最新ログから読む。設定ファイル
ではなくログなのは、実際に効いている状態が要るため。「マイクを挿し直したら
モニタリングが復帰しない」という実障害は、**設定上は正常なまま**起きる。

読んでいる行:

```
16:19:34.828: Audio monitoring device:
16:19:34.828: 	name: BlackHole 2ch                              ← 監視先(次の行にある)
16:19:35.250: coreaudio: Device 'UAB-80' [48000 Hz] initialized  ← マイクが開いた
16:19:35.283: [Loaded global audio device]: 'NoiseC'             ← 音声サブシステム
16:03:40.837: coreaudio: device 'UAB-80' disconnected or changed ← 切断されたまま
21:54:42.835: coreaudio: failed to find device uid: ...UAB-80... ← マイクが繋がっていない
```

起動を待つときは「今回の起動で増えたログファイル」を見る。mtime の新しさで選ぶと、
直前に終了した OBS のログを掴む(終了時刻はたった今なので、どんな猶予を置いても
区別がつかない)。準備完了と見なすのは、音声サブシステムが上がり、かつ入力
デバイスが1つ以上開いたとき。実機ではマイクが開くまで 1 秒かからないことも
16 秒かかることもあった(遅い側は USB マイクが寝ていたとき)ので、前者だけでは
足りない。

監視先(BlackHole 2ch)は入力ではないので、開いた入力の数には入れない。マイクが
不在のときは監視先だけが初期化されるため、数に入れると「マイクが無いのに準備完了」
になる。代わりに `failed to find device uid` を見て**待ちを打ち切る**: この行が
出たら OBS は2秒おきに再試行し続けるだけで、待っても状況は変わらない。打ち切った
うえで「マイクが接続されていません」と理由を出す。

**特定のマイクを前提にしていない。** 名前はログが報告したものをそのまま出すだけで、
OBS のソースがどのデバイスでも同じように動く。ただし **OBS は待つときは uid、
開いたときは表示名で書く**ので、デバイスごとの上書きだけでは「探している」が
解消できない(OBS のソースを別のマイクに差し替えると、古い uid を待つ行がログに
残り続ける)。イベント順に畳み、**監視先以外の入力が開いた時点でそれ以前の
「探している」を消す**ことで解消している。

ログのパースと複数出力装置の構成読みは、実機から取った固定データで
`tests/test_preflight.py` が押さえている。

## できないこと

- **自分側の経路が生きているかの実測**。BlackHole 2ch を録って無音かどうかを見る
  方法は使えない。OBS の RNNoise が無発話区間を落とすため、正常でもデジタル無音に
  なる。会議中の無音検知(`mtg-rec` の `--live-silence-sec`)がこの役目を担う
- **複数出力装置の修復**。構成メンバーの追加は Audio MIDI 設定でしかできない
- **`mtg-rec` / `mtg-live` / `mtg-self` を単体で使うときの点検**。`mtg` からしか
  呼んでいないので、単体で使うなら `mtg-setup` を自分で叩く
