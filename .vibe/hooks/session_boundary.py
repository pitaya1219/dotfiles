#!/usr/bin/env python3
"""
Session Boundary Hook for Mistral Vibe (Global)
...
from session_id_detector import detect_vibe_session_id, get_agent_type

# File-modifying tools for Mistral Vibe
FILE_MODIFYING_TOOLS = ["write_file", "edit"]
...
def main():
    ...
    # Extract data from Vibe format
    tool = input_json.get("tool", "")
    tool_input = input_json.get("input", {})
    provided_session_id = input_json.get("session_id", "")
    
    # Only check file-modifying tools
    if tool not in FILE_MODIFYING_TOOLS:
        sys.exit(0)
    ...
    if not session_id:
        ...
        detected_session_id, _ = detect_vibe_session_id(get_agent_type())
