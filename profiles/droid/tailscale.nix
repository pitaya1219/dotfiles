...
in
{
  ...
  systemd.user.services.tailscaled = {
    ...
    Service = {
      ...
      # Android's kernel ships without the tun module, so a real tailscale0
      # interface can't be created here; route via SOCKS5/HTTP proxy instead.
      ExecStart = "${pkgs.tailscale}/bin/tailscaled --state=%h/.local/share/tailscale/tailscaled.state --socket=%t/tailscale/tailscaled.sock --tun=userspace-networking --socks5-server=localhost:1055 --outbound-http-proxy-listen=localhost:1055";
      Restart = "on-failure";
      RestartSec = "3s";
    };
    Install = {
